...
doc = ["Containing COVID-19 Is Our Highest Priority "]
doc[0]=remove_contractions(doc[0])
doc[0]=clean_text(doc[0])
one_hot_repr_test=[one_hot(words,voc_size)for words in doc]
embeded_docs_test=pad_sequences(one_hot_repr_test,padding='pre',maxlen=sent_length)
predications = model.predict(embeded_docs_test)

# print(predications)
predications = (predications < 0.5)
print(predications)
model.save("onehotCNN.h5")